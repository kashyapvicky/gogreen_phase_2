<?php
  Class Orders_api_model extends CI_Model
  {
     ...
     {
        $this->db->select('ch.services,ch.attendent as status,cd.parking_number,cd.apartment_number,cd.color,cd.reg_no,cb.name as brand,cm.name as model,cl.rating as rating,cl.count_who_rated,ch.car_id,ch.payment_key,st.name');
