	...
	function __construct()
    {
		...
		else
		{
	        ...
	        { 
	            ...
	            break;
	            case 'insert_write_off':
	            $this->insert_write_off($postDataArray);
	            break;
	            case 'change_renewal_mode':
	            $this->change_renewal_mode($postDataArray);
	            break;
	            case 'stop_package':
	            $this->stop_package($postDataArray);
	            break;
	            case 'Cancell_subscription':
	            $this->Cancell_subscription($postDataArray);
	            break;
		...
		else
		{
			...
			else
			{
				$data=array(
					...
					'credited_by'=>'Cleaner',
					'particulars'=>'Payment By Cleaner',
...
//die;
	...
	public function insert_write_off($postDataArray)
	{
		//echo "hello";die;
		$user_id = (isset($postDataArray->user_id) && !empty($postDataArray->user_id)) ? $postDataArray->user_id: '';
		$write_off_amount = (isset($postDataArray->write_off_amount) && !empty($postDataArray->write_off_amount)) ? $postDataArray->write_off_amount: '';
		$reason = (isset($postDataArray->reason) && !empty($postDataArray->reason)) ? $postDataArray->reason: '';
		//$attendent = (isset($postDataArray->attendent) && !empty($postDataArray->attendent)) ? $postDataArray->attendent: '';

		if(empty($user_id) || empty($write_off_amount) || empty($reason))
		{
			$Code = ResponseConstant::UNSUCCESS;
			$rescode = ResponseConstant::UNSUCCESS;
			$Message = ResponseConstant::message('REQUIRED_PARAMETER');
		...
		else
		{
			...
			$insert_id = $this->orders_api_model->insert_write_off($data);
			if($insert_id)
			{
				$Code = ResponseConstant::SUCCESS;
				$rescode = ResponseConstant::SUCCESS;
	...
	public function change_renewal_mode($postDataArray)
	{
		//echo "hello";die;
		$car_id = (isset($postDataArray->car_id) && !empty($postDataArray->car_id)) ? $postDataArray->car_id: '';
		$mode = (isset($postDataArray->mode) && !empty($postDataArray->mode)) ? $postDataArray->mode: '';




		if(empty($car_id) || empty($mode))
		{
			$Code = ResponseConstant::UNSUCCESS;
			$rescode = ResponseConstant::UNSUCCESS;
			$Message = ResponseConstant::message('REQUIRED_PARAMETER');
		...
		{
			//change renewal  mode on this car on current package

			//check if currently package active or not for the same car
			if($mode==3)
			{
				$package_row = $this->orders_api_model->is_currently_package_active($car_id);
				if($package_row)
				{
					$Code = ResponseConstant::SUCCESS;
					$rescode = ResponseConstant::SUCCESS;
					$Message = 'SUCCESFULL';
					$this->sendResponse($Code,$rescode,$Message,$package_row);
				}
				else
				{
					$Code = ResponseConstant::UNSUCCESS;
					$rescode = ResponseConstant::UNSUCCESS;
					$Message = 'NO PACKAGE ACTIVE ON CAR CURRENTLY';
					$this->sendResponse($Code,$rescode,$Message);
			...
			else
			{
				...
				$row = $this->orders_api_model->is_currently_package_active($car_id);
				if($row)
				{
					//echo "<pre>";print_r($row);die;
					$package_id = $row['id'];
					$bool = $this->orders_api_model->update_package_mode($package_id,$mode);
					if($bool)
					{
						$row['auto_renewal']=$mode;
						$Code = ResponseConstant::SUCCESS;
						$rescode = ResponseConstant::SUCCESS;
						$Message = 'MODE CHANGED SUCCESFULLY';
						$this->sendResponse($Code,$rescode,$Message,$row);
					}
					else
					{
						$Code = ResponseConstant::UNSUCCESS;
						$rescode = ResponseConstant::UNSUCCESS;
						$Message = 'MODE  NOT CHANGED ';
						$this->sendResponse($Code,$rescode,$Message,$row);
					}
	...
	public function stop_package($postDataArray)
	{
		$package_id = (isset($postDataArray->package_id) && !empty($postDataArray->package_id)) ? $postDataArray->package_id: '';
		$user_id = (isset($postDataArray->user_id) && !empty($postDataArray->user_id)) ? $postDataArray->user_id: '';
		$mode = (isset($postDataArray->mode) && !empty($postDataArray->mode)) ? $postDataArray->mode: '';
		//1 for on 2 for off


		if(empty($package_id) || empty($mode))
		{
			$Code = ResponseConstant::UNSUCCESS;
			$rescode = ResponseConstant::UNSUCCESS;
			$Message = ResponseConstant::message('REQUIRED_PARAMETER');
		...
		{
			$bool = $this->orders_api_model->update_package_status_is_off($package_id,$mode);
			//echo $this->db->last_query(); die;
			if($bool)
			{
					$user_info = $this->orders_api_model->get_user_info($user_id);
					$name = $user_info['name'];
					...
					{
						$message = "Dear Admin,
							User name ".$name." having phone number ".$phone_number." and email ".$email." Has resume his package;
						 ";
	...
	public function Cancell_subscription($postDataArray)
	{
		...
		else
		{
			...
			$bool  = $this->orders_api_model->stop_user_service($user_id,$status=2);
			if($bool)
			{
				//$user_info = $this->orders_api_model->get_user_info($user_id);

				$row = $this->orders_api_model->get_device_token($user_id);
				$name = $row['name'];
				$phone_number = $row['phone_number'];
				$email = $row['email'];

				$message = "Dear Admin,
							User name ".$name." having phone number ".$phone_number." and email ".$email." Has cancell his Subscription;
						 ";
	...
	public function start_subscription($postDataArray)
	{
		$user_id = (isset($postDataArray->user_id) && !empty($postDataArray->user_id)) ? $postDataArray->user_id: '';
		...
		else
		{
			...
			$bool  = $this->orders_api_model->start_user_service($user_id,$status=1);
			if($bool)
			{
					$Code = ResponseConstant::SUCCESS;
					$rescode = ResponseConstant::SUCCESS;
