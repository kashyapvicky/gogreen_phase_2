...
class Car extends MX_Controller {
	...
	public function get_brand_model_to_edit()
	{
		...
		// $brand_id=1;
		$row = $this->car_model->get_brands_and_model($model_id);
	...
	{
		$brand_id = $this->input->post('brand_id');
		$models = $this->car_model->get_all_models_of_brand($brand_id);
		$output='';
		$output.=' <option disabled selected>Choose A Brand </option>';
