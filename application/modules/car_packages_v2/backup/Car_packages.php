...
class Car_packages extends MY_Controller
{
	...
	public function edit_car_detail($postDataArray)
	{
		...
		if(empty($user_id) || empty($car_id) || empty($brand) || empty($model) || empty($reg_no) || empty($color) ||empty($parking_number) || empty($apartment_number) || empty($city_id) || empty($locality_id) || empty($street_id))
		{
